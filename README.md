...
浏览器半边注册 `tool.call.toolview` 的 `wait_subagent` 键（key 就是 wire 工具名，注册即接管该工具的卡）。卡与通用工具行一样**默认收起**：折叠态只有一行「状态点 + 工具名 + 摘要（`等待 N 个子代理` / `等待结束`）」；**展开后不是参数 JSON，而是一行一个被等待的子代理**——**标签 + 状态 + 最后活动 + 最后一个工具 + 最后一行助手文本**。

- 收起时不订阅、不渲染实时行；**展开才**为名单上每个 id 各开一路 `remote.session.follow` 推送流（开场窗口 4 条消息；不传 `assistantStream`，即只要持久事件、不要 token 级增量），再次收起即 `abort`。**数据全部由推送驱动，没有轮询**。
