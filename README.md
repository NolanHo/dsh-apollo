...
4. **子代理树面板**：会话标题栏的「🌳 子代理」按钮，右下角浮动面板实时显示该会话派生的子代理（含多层后代），点击可打开对应子会话。
...
任意会话的标题栏出现「🌳 子代理」按钮（徽标为已加载层级里的运行中节点数），点击展开右下角浮动面板：

- 目录来自 `sessions` 服务的 `subagentsByParent` 快照，按父会话分层渲染；`hasChildren` 的节点用「▸/▾」逐层展开，展开某一层才拉该层目录（`setSubagentCatalogOpen` 声明"这一层正在被消费"，成员变化由 store 去抖刷新）。
- 面板只对**当前渲染中且 `activity === 'running'`** 的节点开 `remote.session.follow` 流（开场窗口 4 条消息；不传 `assistantStream`，即只要持久事件、不要 token 级增量）：帧到达即折叠出「最后一行助手文本 / 最后一个工具 / 最后活动 N 秒前」。**数据全部由推送驱动，没有轮询**。
- 唯一的时间源是一个每秒的展示时钟：它只重渲染已有时间戳，不取数据。否则「最后活动 N 秒前」会冻结在最后一次推送的时刻，把"正在跑但暂时没有新事件"误读成"停住了"。
- 点击节点名经 `openSubagent(address)` 打开对应子会话。
- 面板关闭、节点停止或从目录消失 → 对应流 `abort`；某路流失败 → 该行显示「读取失败」并 `console.warn`，不自动重试（重连需要退避计时，留待重开面板）。
...
```sh
node --test          # 97 例：技能 provider、捆绑技能目录、预设同步、预设漂移自检、配置读取、退出守卫、wait_subagent、面板路由、子代理树（座位与词典契约、折叠/时长/摊平纯函数、follow 请求形状、流订阅差分与所有权、组件级渲染与生命周期）
```
